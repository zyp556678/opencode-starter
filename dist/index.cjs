...
var require_wrap_ansi = __commonJS({
  "node_modules/wrap-ansi/index.js"(exports2, module2) {
    ...
    var wrapWord = (rows, word, columns) => {
      ...
      let visible = stringWidth3(stripAnsi(rows[rows.length - 1]));
...
var require_package = __commonJS({
  "package.json"(exports2, module2) {
    module2.exports = {
      ...
      version: "1.0.0",
      description: "Interactive terminal launcher for OpenCode with project memory and session management",
      main: "dist/index.cjs",
      bin: {
        ocl: "./bin/ocl.cjs",
        "opencode-launcher": "./bin/ocl.cjs"
      },
      scripts: {
        build: "tsup",
        ...
        shortcut: "node scripts/create-shortcut.js",
...
var globalCleanupRegistered = false;
...
function padToWidth(str, targetWidth) {
  const w = (0, import_string_width.default)(str);
  if (w >= targetWidth) {
    return str;
  }
  return str + " ".repeat(targetWidth - w);
}
function centerString(str, targetWidth) {
  const w = (0, import_string_width.default)(str);
...
async function mouseSelect(options) {
  const {
    bannerLines = [],
    headerLines = [],
    message,
  ...
  const allChoices = [...scrollableChoices, ...fixedList];
  ...
  const rowToChoiceMap = /* @__PURE__ */ new Map();
  function render() {
    ...
    const headerRows = headerLines.length > 0 ? headerLines.length + 1 : 0;
    const promptRows = 2;
    const fixedAreaRows = fixedList.length > 0 ? fixedList.length + 1 : 0;
    const hasAnyDescription = allChoices.some(
      (c) => !("isSeparator" in c) && Boolean(c.description)
    );
    const descDrawerRows = hasAnyDescription ? 2 : 0;
    const cardBottomBorderRows = 1;
    const footerHintRows = 1;
    const scrollIndicatorsRows = 2;
    const safetyBuffer = 2;
    const totalOverhead = topBannerRows + cardTopBorderRows + headerRows + promptRows + fixedAreaRows + descDrawerRows + cardBottomBorderRows + footerHintRows + scrollIndicatorsRows + safetyBuffer;
    ...
    lines.push(padStr + import_picocolors.default.cyan("\u256D" + "\u2500".repeat(cardWidth - 2) + "\u256E"));
    currentScreenRow++;
    if (headerLines.length > 0) {
      for (const h of headerLines) {
        const paddedHeader = padToWidth(h, innerWidth);
        lines.push(padStr + import_picocolors.default.cyan("\u2502 ") + paddedHeader + import_picocolors.default.cyan(" \u2502"));
        currentScreenRow++;
      }
      lines.push(padStr + import_picocolors.default.cyan("\u251C" + "\u2500".repeat(cardWidth - 2) + "\u2524"));
      currentScreenRow++;
    }
    const promptLine = `${import_picocolors.default.cyan("? ")}${import_picocolors.default.bold(message)}`;
    lines.push(padStr + import_picocolors.default.cyan("\u2502 ") + padToWidth(promptLine, innerWidth) + import_picocolors.default.cyan(" \u2502"));
    currentScreenRow++;
    lines.push(padStr + import_picocolors.default.cyan("\u2502 ") + padToWidth(import_picocolors.default.gray("\u2504".repeat(innerWidth)), innerWidth) + import_picocolors.default.cyan(" \u2502"));
    currentScreenRow++;
    if (scrollableChoices.length > 0) {
      if (scrollWindowStart > 0) {
        const topScroll = import_picocolors.default.yellow(`  \u2191 ... (\u5411\u4E0A\u6ED1\u52A8\u6EDA\u8F6E\u67E5\u770B\u66F4\u591A)`);
        lines.push(padStr + import_picocolors.default.cyan("\u2502 ") + padToWidth(topScroll, innerWidth) + import_picocolors.default.cyan(" \u2502"));
        currentScreenRow++;
      }
      for (let i = scrollWindowStart; i < scrollWindowEnd; i++) {
        const item = scrollableChoices[i];
        if ("isSeparator" in item) {
          const sepContent = import_picocolors.default.gray("\u2500".repeat(innerWidth));
          lines.push(padStr + import_picocolors.default.cyan("\u2502 ") + padToWidth(sepContent, innerWidth) + import_picocolors.default.cyan(" \u2502"));
        } else if (item.disabled) {
          const disContent = import_picocolors.default.gray(`   - ${item.name} (\u4E0D\u53EF\u7528)`);
          lines.push(padStr + import_picocolors.default.cyan("\u2502 ") + padToWidth(disContent, innerWidth) + import_picocolors.default.cyan(" \u2502"));
        } else {
          const isSelected = i === activeGlobalIdx;
          const prefix = isSelected ? import_picocolors.default.cyan("\u276F ") : "  ";
          const textFormatted = isSelected ? import_picocolors.default.cyan(import_picocolors.default.bold(item.name)) : item.name;
          const fullContent = `${prefix}${textFormatted}`;
          lines.push(padStr + import_picocolors.default.cyan("\u2502 ") + padToWidth(fullContent, innerWidth) + import_picocolors.default.cyan(" \u2502"));
          rowToChoiceMap.set(currentScreenRow, i);
        }
        currentScreenRow++;
      }
      if (scrollWindowEnd < scrollableChoices.length) {
        const botScroll = import_picocolors.default.yellow(`  \u2193 ... (\u5411\u4E0B\u6ED1\u52A8\u6EDA\u8F6E\u67E5\u770B\u66F4\u591A)`);
        lines.push(padStr + import_picocolors.default.cyan("\u2502 ") + padToWidth(botScroll, innerWidth) + import_picocolors.default.cyan(" \u2502"));
        currentScreenRow++;
      }
    }
    ...
    if (activeItem && !("isSeparator" in activeItem) && activeItem.description) {
      lines.push(padStr + import_picocolors.default.cyan("\u251C" + "\u2500".repeat(cardWidth - 2) + "\u2524"));
      currentScreenRow++;
      const descLine = import_picocolors.default.gray(`\u21B3 ${activeItem.description}`);
      lines.push(padStr + import_picocolors.default.cyan("\u2502 ") + padToWidth(descLine, innerWidth) + import_picocolors.default.cyan(" \u2502"));
      currentScreenRow++;
    }
    lines.push(padStr + import_picocolors.default.cyan("\u2570" + "\u2500".repeat(cardWidth - 2) + "\u256F"));
    currentScreenRow++;
    const hintText = import_picocolors.default.gray("[ \u{1F5B1}\uFE0F \u6EDA\u8F6E\u6ED1\u52A8 \xB7 \u5355\u51FB\u76F4\u63A5\u8FDB\u5165 ]     [ \u2328\uFE0F \u2191\u2193\u79FB\u52A8 \xB7 Enter\u786E\u8BA4 \xB7 Esc/q\u8FD4\u56DE ]");
  ...
  function confirmChoiceIndex(idx) {
...
var origEmit = process.emit.bind(process);
...
function getDbPath() {
  ...
  return import_node_path2.default.join(import_node_os2.default.homedir(), ".local", "share", "opencode", "opencode.db");
}
function fetchSessionsFromSqlite(dbFilePath) {
  try {
    ...
    try {
      ...
      return rows.map((r) => ({
        ...
        directory: import_node_path2.default.normalize(String(r.directory)),
...
function fetchSessionsFromCli() {
  try {
    ...
    if (process.platform === "win32") {
      ...
      stdout = (0, import_node_child_process.execFileSync)(comspec, ["/c", "opencode", "session", "list", "--format", "json"], {
        encoding: "utf8",
        timeout: 1e4,
        stdio: ["ignore", "pipe", "ignore"]
      });
    } else {
      stdout = (0, import_node_child_process.execFileSync)("opencode", ["session", "list", "--format", "json"], {
        encoding: "utf8",
        timeout: 1e4,
        stdio: ["ignore", "pipe", "ignore"]
      });
    ...
    return parsed.map((item) => ({
      ...
      directory: import_node_path2.default.normalize(String(item.directory)),
...
function deleteSession(sessionId) {
  try {
    if (process.platform === "win32") {
      const comspec = process.env.COMSPEC || "cmd.exe";
      (0, import_node_child_process.execFileSync)(comspec, ["/c", "opencode", "session", "delete", sessionId], {
        encoding: "utf8",
        timeout: 15e3,
        stdio: ["ignore", "pipe", "pipe"]
      });
    } else {
      (0, import_node_child_process.execFileSync)("opencode", ["session", "delete", sessionId], {
        encoding: "utf8",
        timeout: 15e3,
        stdio: ["ignore", "pipe", "pipe"]
      });
...
var CONFIG_DIR = import_node_path3.default.join(import_node_os3.default.homedir(), ".config", "opencode-launcher");
var CONFIG_FILE = import_node_path3.default.join(CONFIG_DIR, "config.json");
var DEFAULT_CONFIG = {
  pinnedPaths: [],
  customPaths: [],
  hiddenPaths: [],
...
function loadConfig() {
  try {
    ...
    const raw = import_node_fs2.default.readFileSync(CONFIG_FILE, "utf8");
    ...
    return {
      ...
      lastOpenedPath: typeof parsed.lastOpenedPath === "string" ? parsed.lastOpenedPath : void 0,
...
function saveConfig(config) {
  try {
    ensureDirSync(CONFIG_DIR);
    import_node_fs2.default.writeFileSync(CONFIG_FILE, JSON.stringify(config, null, 2), "utf8");
  } catch (err) {
    console.error("\u4FDD\u5B58\u914D\u7F6E\u6587\u4EF6\u5931\u8D25:", err);
  }
}
function recordFolderAccess(folderPath) {
  const config = loadConfig();
  const normalized = import_node_path3.default.normalize(folderPath);
  config.lastOpenedPath = normalized;
  if (!config.customPaths.includes(normalized)) {
    config.customPaths.push(normalized);
  }
  config.hiddenPaths = config.hiddenPaths.filter((p) => import_node_path3.default.normalize(p) !== normalized);
  const existingIdx = config.folderHistory.findIndex((item) => import_node_path3.default.normalize(item.path) === normalized);
  if (existingIdx >= 0) {
...
function togglePinProject(folderPath) {
  const config = loadConfig();
  const normalized = import_node_path3.default.normalize(folderPath);
  const isPinned = config.pinnedPaths.some((p) => import_node_path3.default.normalize(p) === normalized);
  if (isPinned) {
    config.pinnedPaths = config.pinnedPaths.filter((p) => import_node_path3.default.normalize(p) !== normalized);
  } else {
...
function hideProject(folderPath) {
  const config = loadConfig();
  const normalized = import_node_path3.default.normalize(folderPath);
  if (!config.hiddenPaths.some((p) => import_node_path3.default.normalize(p) === normalized)) {
    config.hiddenPaths.push(normalized);
  }
  config.pinnedPaths = config.pinnedPaths.filter((p) => import_node_path3.default.normalize(p) !== normalized);
  config.customPaths = config.customPaths.filter((p) => import_node_path3.default.normalize(p) !== normalized);
  saveConfig(config);
}
...
function resolveFolderPath(inputPath) {
  ...
  if (resolved.startsWith("~")) {
    resolved = import_node_path4.default.join(import_node_os4.default.homedir(), resolved.slice(1));
  }
  return import_node_path4.default.resolve(resolved);
}
function isValidDirectory(folderPath) {
  try {
    if (!import_node_fs3.default.existsSync(folderPath)) return false;
    return import_node_fs3.default.statSync(folderPath).isDirectory();
  } catch {
    return false;
  }
}
function truncatePath(fullPath, maxLength = 45) {
  if (fullPath.length <= maxLength) return fullPath;
  const parts = fullPath.split(/[\\/]/);
  if (parts.length <= 2) return fullPath;
  const first = parts[0] + import_node_path4.default.sep + parts[1];
  const last = parts[parts.length - 1];
  const middle = "...";
  const candidate = `${first}${import_node_path4.default.sep}${middle}${import_node_path4.default.sep}${last}`;
...
`.trim();
    ...
    const result = (0, import_node_child_process2.execFileSync)("powershell.exe", ["-NoProfile", "-STA", "-EncodedCommand", base64], {
      encoding: "utf8",
      timeout: 6e4
    }).trim();
    return result && isValidDirectory(result) ? import_node_path4.default.normalize(result) : null;
  } catch {
    return null;
  }
}
...
function normalizeDir(d) {
  return import_node_path5.default.normalize(d);
}
function dirEquals(a, b) {
  return import_node_path5.default.normalize(a).toLowerCase() === import_node_path5.default.normalize(b).toLowerCase();
}
function getAllProjects() {
  ...
  const directoriesSet = /* @__PURE__ */ new Set();
  ...
  for (const dir of activeDirectories) {
    ...
    const displayName = import_node_path5.default.basename(dir) || dir;
...
async function launchOpenCode(options) {
  const { directory, mode, sessionId } = options;
  recordFolderAccess(directory);
  const displayName = import_node_path6.default.basename(directory) || directory;
  ...
  console.log();
  console.log(import_picocolors2.default.green(`\u{1F680} \u6B63\u5728\u5F00\u542F\u65B0\u72EC\u7ACB\u7A97\u53E3\u8FD0\u884C OpenCode...`));
  console.log(import_picocolors2.default.gray(`\u{1F4C1} \u5DE5\u4F5C\u76EE\u5F55: ${directory}`));
  if (sessionId) {
    console.log(import_picocolors2.default.gray(`\u{1F4AC} \u4F1A\u8BDD ID: ${sessionId}`));
  } else if (mode === "continue") {
    console.log(import_picocolors2.default.gray(`\u26A1 \u6A21\u5F0F: \u7EE7\u7EED\u6700\u65B0\u4F1A\u8BDD (-c)`));
  } else {
    console.log(import_picocolors2.default.gray(`\u2728 \u6A21\u5F0F: \u65B0\u5EFA\u4F1A\u8BDD`));
  }
  if (process.platform === "win32") {
    try {
      let wtFailed = false;
      const wtChild = (0, import_node_child_process3.spawn)(
        "wt.exe",
        [
          "--window",
          "new",
    ...
    try {
      const comspec = process.env.COMSPEC || "cmd.exe";
      const cmdChild = (0, import_node_child_process3.spawn)(
        comspec,
      ...
      console.error(import_picocolors2.default.red(`\u274C \u542F\u52A8\u65B0\u7A97\u53E3\u5931\u8D25: ${err.message}`));
      return false;
    }
  } else if (process.platform === "darwin") {
    const script = `tell application "Terminal" to do script "cd '${directory}' && ${opencodeCmdString}"`;
    (0, import_node_child_process3.spawn)("osascript", ["-e", script], { detached: true, stdio: "ignore" }).unref();
    return true;
  } else {
    (0, import_node_child_process3.spawn)("x-terminal-emulator", ["-e", `sh -c "cd '${directory}' && ${opencodeCmdString}"`], {
      detached: true,
      stdio: "ignore"
    }).unref();
    return true;
...
function padColumn(str, targetWidth) {
  const w = (0, import_string_width2.default)(str);
  if (w >= targetWidth) return str;
  return str + " ".repeat(targetWidth - w);
}
...
async function showMainMenu() {
  ...
  const bannerLines = [
    ...
    import_picocolors3.default.cyan("\u2502") + import_picocolors3.default.bold(import_picocolors3.default.white("                 \u26A1 OpenCode \u7EC8\u7AEF\u542F\u52A8\u5668                      ")) + import_picocolors3.default.cyan("\u2502"),
    import_picocolors3.default.cyan("\u2502") + import_picocolors3.default.gray("              \u9879\u76EE\u5FEB\u901F\u9009\u62E9 \xB7 \u4F1A\u8BDD\u8BB0\u5FC6\u4E0E\u7BA1\u7406                  ") + import_picocolors3.default.cyan("\u2502"),
    import_picocolors3.default.cyan("\u2514\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2518")
  ];
  const headerLines = [
    `${import_picocolors3.default.cyan("\u{1F4C1} \u5386\u53F2\u9879\u76EE\u9762\u677F")}  ${import_picocolors3.default.gray(`(\u5171 ${projects.length} \u4E2A\u5DF2\u8BB0\u5FC6\u9879\u76EE)`)}`
  ];
  const choices = [];
  if (projects.length > 0) {
    for (const p of projects) {
      const pinPrefix = p.pinned ? import_picocolors3.default.yellow("\u{1F4CC} ") : "\u{1F4C2} ";
      const statusSuffix = !p.exists ? import_picocolors3.default.red(" [\u5DF2\u5931\u6548]") : "";
      const sessionCount = p.sessions.length;
      const sessionInfo = sessionCount > 0 ? import_picocolors3.default.cyan(`[${sessionCount}\u4F1A\u8BDD \xB7 ${formatRelativeTime(p.lastActive)}]`) : import_picocolors3.default.gray("[\u65E0\u4F1A\u8BDD]");
      const colName = padColumn(`${pinPrefix}${import_picocolors3.default.bold(p.displayName)}`, 18);
      const colPath = padColumn(import_picocolors3.default.gray(truncatePath(p.directory, 28)), 30);
      const colBadge = `${sessionInfo}${statusSuffix}`;
      const latestTitle = p.sessions.length > 0 ? `\u6700\u65B0: ${p.sessions[0].title}` : "\u65E0\u5386\u53F2\u4F1A\u8BDD";
      choices.push({
    ...
    choices.push(new Separator2(import_picocolors3.default.yellow("\u26A0\uFE0F \u6682\u672A\u68C0\u6D4B\u5230\u5386\u53F2\u9879\u76EE\uFF0C\u8BF7\u4F7F\u7528\u4E0B\u65B9\u9009\u9879\u6D4F\u89C8\u9009\u62E9\u9879\u76EE\u6587\u4EF6\u5939")));
  }
  const fixedChoices = [];
  if (process.platform === "win32") {
    fixedChoices.push({
      name: import_picocolors3.default.blue("\u{1FA9F}  Windows \u5F39\u7A97\u6D4F\u89C8\u9009\u62E9\u6587\u4EF6\u5939..."),
  ...
  if (projects.length > 0) {
    fixedChoices.push({
      name: import_picocolors3.default.cyan("\u{1F4CB}  \u67E5\u770B\u5168\u90E8\u5386\u53F2\u4F4D\u7F6E\u6E05\u5355 (\u5B8C\u6574\u7EDD\u5BF9\u8DEF\u5F84\u5217\u8868)"),
  ...
  if (projects.length > 3) {
    fixedChoices.push({
      name: import_picocolors3.default.magenta("\u{1F50D}  \u5FEB\u901F\u641C\u7D22\u9879\u76EE"),
      value: { type: "action", action: "search" },
      description: "\u6309\u540D\u79F0\u6216\u76EE\u5F55\u8FC7\u6EE4\u9879\u76EE"
    });
  }
  fixedChoices.push({
    name: import_picocolors3.default.gray("\u274C  \u9000\u51FA\u542F\u52A8\u5668"),
...
\u{1F4C1} \u9879\u76EE: `) + import_picocolors3.default.bold(project.displayName));
      console.log(import_picocolors3.default.gray(`   \u8DEF\u5F84: ${project.directory}`));
      console.log(import_picocolors3.default.yellow("\u26A0\uFE0F \u8BE5\u76EE\u5F55\u5728\u78C1\u76D8\u4E0A\u5DF2\u4E0D\u5B58\u5728\uFF0C\u65E0\u6CD5\u76F4\u63A5\u542F\u52A8\u3002"));
      ...
      if (removeAns) {
        ...
        console.log(import_picocolors3.default.green("\u2705 \u5DF2\u4ECE\u8BB0\u5F55\u4E2D\u79FB\u9664"));
      }
      return;
    }
    const headerLines = [
      `${import_picocolors3.default.cyan("\u{1F4C1} \u5F53\u524D\u9879\u76EE:")} ${import_picocolors3.default.bold(import_picocolors3.default.white(project.displayName))}`,
      `${import_picocolors3.default.gray("\u{1F4CD} \u7269\u7406\u8DEF\u5F84:")} ${import_picocolors3.default.white(truncatePath(project.directory, 56))}`,
      `${import_picocolors3.default.gray("\u{1F4CA} \u9879\u76EE\u72B6\u6001:")} ${project.exists ? import_picocolors3.default.green("\u25CF \u8DEF\u5F84\u6709\u6548") : import_picocolors3.default.red("\u25CB \u8DEF\u5F84\u5DF2\u5931\u6548")}   ${import_picocolors3.default.gray("\u2502   \u5386\u53F2\u4F1A\u8BDD:")} ${import_picocolors3.default.cyan(project.sessions.length + " \u4E2A")}`
    ];
    const choices = [];
    if (project.sessions.length > 0) {
      const latest = project.sessions[0];
      choices.push({
        name: import_picocolors3.default.green(`\u26A1 \u7EE7\u7EED\u6700\u65B0\u4F1A\u8BDD`) + import_picocolors3.default.gray(`: "${truncatePath(latest.title, 35)}"`),
        value: "continue",
        description: `\u4F1A\u8BDD ID: ${latest.id} (${formatRelativeTime(latest.updated)})`
      });
    }
    choices.push({
      name: import_picocolors3.default.cyan("\u2728 \u5F00\u542F\u5168\u65B0\u4F1A\u8BDD"),
      value: "new",
      description: "\u5728\u8BE5\u76EE\u5F55\u4E0B\u542F\u52A8\u4E00\u4E2A\u5168\u65B0\u7684\u7A7A\u767D\u4F1A\u8BDD"
    });
    if (project.sessions.length > 0) {
      choices.push({
        name: import_picocolors3.default.yellow(`\u{1F4DC} \u6D4F\u89C8\u5168\u90E8\u5386\u53F2\u4F1A\u8BDD (${project.sessions.length}\u4E2A)`),
        value: "sessions",
        description: "\u9009\u62E9\u6307\u5B9A\u7684\u5386\u53F2\u5BF9\u8BDD\u8FDB\u884C\u6062\u590D"
      });
      choices.push({
        name: import_picocolors3.default.red("\u{1F5D1}\uFE0F \u5220\u9664\u5386\u53F2\u4F1A\u8BDD (Ctrl+D)"),
        value: "delete_session",
        description: "\u9009\u62E9\u5E76\u6C38\u4E45\u5220\u9664\u8BE5\u9879\u76EE\u4E0B\u7684\u6307\u5B9A\u5386\u53F2\u4F1A\u8BDD"
      });
    }
    choices.push(new Separator2());
    choices.push({
      name: project.pinned ? import_picocolors3.default.gray("\u{1F4CC} \u53D6\u6D88\u7F6E\u9876\u6B64\u9879\u76EE") : import_picocolors3.default.yellow("\u{1F4CC} \u7F6E\u9876\u6B64\u9879\u76EE"),
      value: "pin"
    });
    if (process.platform === "win32") {
      choices.push({
        name: import_picocolors3.default.blue("\u{1F4C2} \u5728\u8D44\u6E90\u7BA1\u7406\u5668\u4E2D\u6253\u5F00\u6B64\u6587\u4EF6\u5939"),
        value: "explorer"
      });
    }
    choices.push({
      name: import_picocolors3.default.red("\u{1F5D1}\uFE0F \u4ECE\u5386\u53F2\u8BB0\u5F55\u4E2D\u79FB\u9664"),
      value: "remove"
    });
    choices.push({
      name: import_picocolors3.default.gray("\u{1F519} \u8FD4\u56DE\u9879\u76EE\u5217\u8868"),
      value: "back"
    });
    ...
    if (action === "pin") {
      ...
      console.log(import_picocolors3.default.green(nowPinned ? "\u2705 \u5DF2\u7F6E\u9876\u8BE5\u9879\u76EE" : "\u2705 \u5DF2\u53D6\u6D88\u7F6E\u9876"));
    }
    if (action === "explorer") {
      (0, import_node_child_process4.exec)(`explorer "${project.directory}"`);
    }
    if (action === "remove") {
      hideProject(project.directory);
      console.log(import_picocolors3.default.green("\u2705 \u5DF2\u4ECE\u8BB0\u5F55\u4E2D\u79FB\u9664"));
...
async function handleSelectSession(project) {
  while (project.sessions.length > 0) {
    const headerLines = [
      `${import_picocolors3.default.cyan("\u{1F4DC} \u5386\u53F2\u4F1A\u8BDD\u6E05\u5355")} \u2500\u2500 ${import_picocolors3.default.bold(import_picocolors3.default.white(project.displayName))}`,
      `${import_picocolors3.default.gray("\u{1F4CA} \u5BF9\u8BDD\u603B\u8BA1:")} ${import_picocolors3.default.cyan(project.sessions.length + " \u4E2A\u5386\u53F2\u5BF9\u8BDD")}   ${import_picocolors3.default.gray("\u2502   \u76EE\u5F55:")} ${import_picocolors3.default.gray(truncatePath(project.directory, 45))}`
    ];
    const choices = [];
    for (const s of project.sessions) {
      const timeStr = formatRelativeTime(s.updated);
      const colTime = padColumn(import_picocolors3.default.cyan(`[${timeStr}]`), 14);
      const colTitle = truncatePath(s.title, 50);
      choices.push({
        name: `\u{1F4AC} ${colTime} ${colTitle}`,
    ...
    const fixedChoices = [
      {
        name: import_picocolors3.default.red("\u{1F5D1}\uFE0F  \u5220\u9664\u6307\u5B9A\u5386\u53F2\u4F1A\u8BDD (Ctrl+D)"),
        value: { type: "delete_mode" },
        description: "\u8FDB\u5165\u4F1A\u8BDD\u5220\u9664\u6A21\u5F0F\uFF0C\u9009\u62E9\u5E76\u5F7B\u5E95\u9500\u6BC1\u6307\u5B9A\u4F1A\u8BDD"
      },
      {
        name: import_picocolors3.default.gray("\u{1F519} \u8FD4\u56DE\u4E0A\u4E00\u7EA7"),
        value: { type: "back" }
      }
    ];
...
async function handleDeleteSessionFlow(project) {
  while (project.sessions.length > 0) {
    const headerLines = [
      `${import_picocolors3.default.red("\u{1F5D1}\uFE0F  \u6C38\u4E45\u5220\u9664\u4F1A\u8BDD\u6A21\u5F0F (Ctrl+D)")} \u2500\u2500 ${import_picocolors3.default.bold(import_picocolors3.default.white(project.displayName))}`,
      import_picocolors3.default.yellow("\u26A0\uFE0F  \u8B66\u544A\uFF1A\u5728\u6B64\u5904\u9009\u4E2D\u7684\u4F1A\u8BDD\u5C06\u88AB\u6C38\u4E45\u62B9\u9664\uFF0C\u8BF7\u8C28\u614E\u9009\u62E9\uFF01")
    ];
    const choices = [];
    for (const s of project.sessions) {
      const timeStr = formatRelativeTime(s.updated);
      const colTime = padColumn(import_picocolors3.default.gray(`(${timeStr})`), 14);
      const colTitle = truncatePath(s.title, 48);
      choices.push({
        name: `${import_picocolors3.default.red("\u{1F5D1}\uFE0F ")} ${import_picocolors3.default.white(colTitle)} ${colTime}`,
        value: s,
        description: `\u4F1A\u8BDD ID: ${s.id} | \u66F4\u65B0\u65F6\u95F4: ${new Date(s.updated).toLocaleString()}`
      });
    }
    const fixedChoices = [
      {
        name: import_picocolors3.default.gray("\u{1F519} \u9000\u51FA\u5220\u9664\u6A21\u5F0F\uFF0C\u8FD4\u56DE\u4E0A\u4E00\u7EA7"),
        value: null
      }
    ];
    ...
    console.log();
    console.log(import_picocolors3.default.yellow("\u26A0\uFE0F  \u8B66\u544A\uFF1A\u6B64\u64CD\u4F5C\u4E0E OpenCode \u539F\u751F Ctrl+D \u76F8\u540C\uFF0C\u5C06\u6C38\u4E45\u5220\u9664\u8BE5\u4F1A\u8BDD\uFF01"));
    console.log(import_picocolors3.default.white(`   \u4F1A\u8BDD\u6807\u9898: ${import_picocolors3.default.bold(target.title)}`));
    console.log(import_picocolors3.default.gray(`   \u4F1A\u8BDD\u7F16\u53F7: ${target.id}`));
    console.log(import_picocolors3.default.gray(`   \u6700\u540E\u6D3B\u8DC3: ${new Date(target.updated).toLocaleString()}`));
    console.log();
    const confirmed = await safePrompt(
      () => esm_default4({
        message: import_picocolors3.default.red("\u662F\u5426\u786E\u8BA4\u5F7B\u5E95\u5220\u9664\u6B64\u4F1A\u8BDD\uFF1F"),
        default: false
      })
    );
    if (confirmed) {
      ...
      if (result.success) {
        ...
        console.log(import_picocolors3.default.green(`\u2705 \u5DF2\u6210\u529F\u5F7B\u5E95\u5220\u9664\u4F1A\u8BDD: "${target.title}"
`));
      } else {
        console.error(import_picocolors3.default.red(`\u274C \u5220\u9664\u5931\u8D25: ${result.error || "\u672A\u77E5\u9519\u8BEF"}
`));
      }
      await safePrompt(
        () => esm_default5({
      ...
      console.log(import_picocolors3.default.gray("\u5DF2\u53D6\u6D88\u5220\u9664\u3002\n"));
    }
  }
  console.log(import_picocolors3.default.yellow("\u8BE5\u9879\u76EE\u5DF2\u65E0\u5386\u53F2\u4F1A\u8BDD\u3002"));
  await safePrompt(
    () => esm_default5({
      message: "\u6309\u56DE\u8F66\u952E\u8FD4\u56DE..."
    })
...
async function handleWindowsPicker() {
  console.log(import_picocolors3.default.cyan("\n\u6B63\u5728\u6253\u5F00 Windows \u6587\u4EF6\u5939\u9009\u62E9\u5668..."));
  const picked = openWindowsFolderPicker();
  if (!picked) {
    console.log(import_picocolors3.default.gray("\u672A\u9009\u62E9\u4EFB\u4F55\u6587\u4EF6\u5939\u6216\u5DF2\u53D6\u6D88\u3002"));
    return;
  }
  console.log(import_picocolors3.default.green(`\u2705 \u5DF2\u9009\u62E9\u6587\u4EF6\u5939: ${picked}`));
  recordFolderAccess(picked);
  const all = getAllProjects();
  const found = all.find((p) => p.directory.toLowerCase() === picked.toLowerCase()) || {
    directory: picked,
    displayName: import_node_path7.default.basename(picked) || picked,
    exists: true,
    pinned: false,
    lastActive: Date.now(),
    sessions: []
...
async function handleSearchProjects(projects) {
  ...
  if (matched.length === 0) {
    console.log(import_picocolors3.default.yellow(`\u672A\u627E\u5230\u5305\u542B "${keyword}" \u7684\u9879\u76EE\u3002`));
    await safePrompt(
      () => esm_default5({
        message: "\u6309\u56DE\u8F66\u7EE7\u7EED..."
      })
  ...
  const choices = matched.map((p) => ({
    name: `${p.pinned ? "\u{1F4CC} " : "\u{1F4C2} "}${import_picocolors3.default.bold(p.displayName)} ${import_picocolors3.default.gray(`(${p.sessions.length}\u4E2A\u4F1A\u8BDD)`)}`,
    value: p,
    description: p.directory
  }));
  const headerLines = [
    import_picocolors3.default.cyan("\u{1F50D} \u641C\u7D22\u7ED3\u679C: ") + import_picocolors3.default.bold(`\u627E\u5230 ${matched.length} \u4E2A\u5339\u914D\u9879\u76EE`),
    ""
  ];
  const selected = await safePrompt(
    () => mouseSelect({
...
async function handleViewAllLocations(projects) {
  if (projects.length === 0) {
    console.clear();
    console.log(import_picocolors3.default.yellow("\u6682\u65E0\u4EFB\u4F55\u8BB0\u5FC6\u7684\u9879\u76EE\u4F4D\u7F6E\u3002\n"));
    await safePrompt(
      () => esm_default5({
        message: "\u6309\u56DE\u8F66\u8FD4\u56DE\u4E3B\u83DC\u5355..."
      })
  ...
  const headerLines = [
    `${import_picocolors3.default.cyan("\u{1F4CB} \u5168\u90E8\u5DF2\u8BB0\u5FC6\u7684\u5386\u53F2\u9879\u76EE\u4F4D\u7F6E\u6E05\u5355")}  ${import_picocolors3.default.gray(`(\u5171 ${projects.length} \u4E2A\u9879\u76EE)`)}`
  ];
  const choices = projects.map((p, idx) => {
    const num = import_picocolors3.default.cyan(`[${idx + 1}]`);
    const pinTag = p.pinned ? import_picocolors3.default.yellow("\u{1F4CC} ") : "\u{1F4C1} ";
    const statusTag = p.exists ? import_picocolors3.default.green("\u25CF \u8DEF\u5F84\u6709\u6548") : import_picocolors3.default.red("\u25CB \u8DEF\u5F84\u5DF2\u5931\u6548");
    return {
      name: `${num} ${pinTag}${import_picocolors3.default.bold(p.displayName)}  ${import_picocolors3.default.cyan(`(${p.sessions.length} \u4F1A\u8BDD)`)}  ${statusTag}`,
      value: p,
      description: `${p.directory}  |  \u6700\u8FD1\u6D3B\u8DC3: ${formatRelativeTime(p.lastActive)}`
    };
  });
  choices.push({
    name: import_picocolors3.default.gray("\u{1F519} \u8FD4\u56DE\u4E3B\u83DC\u5355"),
    value: null
  });
...
// src/index.ts
var origEmit2 = process.emit.bind(process);
process.emit = function(name, data, ...rest) {
  if (name === "warning" && data && (data.name === "ExperimentalWarning" || data.name === "DeprecationWarning" || data.message?.includes("DEP0190") || data.message?.includes("SQLite"))) {
    return false;
  }
  return origEmit2(name, data, ...rest);
};
async function main() {
  const args = process.argv.slice(2);
  if (args.includes("-h") || args.includes("--help")) {
    console.log(`
${import_picocolors4.default.bold(import_picocolors4.default.cyan("OpenCode Launcher (ocl)"))} - \u7EC8\u7AEF\u4EA4\u4E92\u5F0F\u9879\u76EE\u542F\u52A8\u5668

${import_picocolors4.default.bold("\u7528\u6CD5:")}
  ocl                 \u6253\u5F00\u4EA4\u4E92\u5F0F\u9879\u76EE\u9009\u62E9\u4E0E\u4F1A\u8BDD\u7BA1\u7406\u83DC\u5355
  ocl <\u76EE\u5F55\u8DEF\u5F84>      \u76F4\u63A5\u6253\u5F00\u6307\u5B9A\u76EE\u5F55\u5E76\u8FDB\u5165\u64CD\u4F5C\u83DC\u5355
  ocl --help, -h      \u663E\u793A\u5E2E\u52A9\u4FE1\u606F
  ocl --version, -v   \u663E\u793A\u7248\u672C\u53F7

${import_picocolors4.default.bold("\u7279\u6027:")}
  \u2022 \u81EA\u52A8\u8BB0\u5FC6\u5386\u53F2\u9879\u76EE\u8DEF\u5F84\u4E0E\u81EA\u5B9A\u4E49\u7F6E\u9876
...
`);
  ...
  try {
    ...
    console.error(import_picocolors4.default.red(`\u8FD0\u884C\u65F6\u5F02\u5E38: ${err.message}`));
    process.exit(1);
  }
}
main();
